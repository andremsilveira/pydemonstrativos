...
for ano in lista_anos:
    lista_files.append(link.format(ano))
...
for link in links_cvm_sem_data:
    for ano in lista_anos:
...
def listar_empresas(ano):
    link_empresa = [sentence for sentence in links_gcloud if all(w in sentence for w in ["BPA", str(ano_analise)])][0]
    dataframe_empresa = pd.read_csv(a, encoding='ISO-8859-1', sep=';')
    for empresa in dataframe_empresa['DENOM_CIA'].unique():
        print(empresa)
